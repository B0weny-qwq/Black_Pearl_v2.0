...
STC32G 官方驱动和官方示例参考价值的前提下，把真实项目代码迁移到更清晰
...
- 官方 STC 示例：`Examples/STC32G_Official/`
- 官方 STC 驱动：`Drivers/`

官方示例仅作为参考代码保留，不参与当前 Keil 工程构建。
...
- `McuAbstraction/` 保存基于 STC SDK 的统一外设抽象接口，例如 `ef_uart`、
  `ef_iic`、`ef_spi`。
- `Examples/` 保存可复制参考的官方示例代码，允许包含 vendor 头文件。
...
1. 从 `Examples/STC32G_Official/` 找到需要参考的官方示例逻辑。
...
官方示例中的 UART2 桥接回显属于 sample app 行为，未放入 SPI-PS 抽象层。
