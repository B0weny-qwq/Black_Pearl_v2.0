...
本工程是面向 Black Pearl v1.1 的移植、解耦和重构版本。目标是在保留
STC32G 官方驱动参考价值的前提下，把真实项目代码迁移到更清晰
的 EmbedForge Level 1.5 工程结构中。
文件目录在 "C:\Users\S\Desktop\STC_PROJECT\Black_Pearl_v1.1"
...
- 船控逻辑：Code_boweny/Device/Control/ShipControl.c
- 自动驾驶：Code_boweny/Device/AutoDrive/autodrive.c
- 无线协议/上位机数据格式：Code_boweny/Device/WIRELESS/ship_protocol.c
- AHRS/航向估计：Code_boweny/Function/AHRS/
- 主循环/功能开关：User/MainLoop.c、User/FeatureSwitch.h

## 工程目标

- 严格参考 Black Pearl v1.1 的算法解算、控制逻辑、业务逻辑和通信数据格式，
  同时保留 STC 官方驱动语义，降低移植成本。
...
- 改电机控制策略：看 `App/Src/ship_control*.c`，最终输出仍必须走 `board_motor_set_both_speed()`；手动逻辑在 `ship_control_manual.c`，航向保持/yaw 阻尼在 `ship_control_yaw.c`。
- 改无线协议、按键语义、GPS 回包：看 `App/Src/ship_protocol*.c`；命令分发在 `ship_protocol_cmd.c`，0x12/0x16 回包在 `ship_protocol_tx.c`。
- 改返航、钓点、GPS 定点逻辑：看 `App/Src/autodrive*.c`；点位表在 `autodrive_points.c`，距离/方位在 `autodrive_nav.c`，状态机轮询在 `autodrive_poll.c`。
- 改 AHRS、滤波、PID、航向算法：改 `Components/`，不要访问硬件。

App 层已经按职责拆分，单个 `.c/.h` 文件控制在 300 行以内。对接时先看对应
`README.md` 和 `*_internal.h`，不要把新增业务重新塞回某一个大文件。
...
- yaw 阻尼严格按 v1.1 证据走 Z 轴角速度：旧工程 `ShipControl_ApplyYawHoldDamping()` 调用
  `MainLoop_GetGyroZDps100()`，而 `User/MainLoop.c` 的快照来自 `att->gyro_z_dps100`。
  当前实现对应 `App/Src/ship_control_yaw.c` 的 `att->gyro_z_dps100`，不是 `gyro_y_dps100`。
...
- `Services/Inc/Log.h`：旧代码 include 兼容入口，后续迁移 v1.1 代码时可临时
  保持 `#include "Log.h"`。
...
- `App/Src/app_bringup.c` now performs `board_gps_init()` during bring-up and logs the init return code at startup.
- `BoardDevices/Src/board_gps.c` and `ChipDrivers/Src/gnss_nmea.c` already expose the old upper-layer dependent GPS fields, including:
  ...
  - `course_deg_x100`
  - `satellites_used_gsa`
- `App/Src/ship_protocol_tx.c` `0x12` payload now follows the old report semantics:
...
- `App/Src/app_mag_event.c` now drains protocol events in `app_ship_event_poll()` each loop. High-rate throttle/power sample events stay mostly quiet, while key/action/point/power-latch/SPI-PS/error events have explicit dispatch logs.
